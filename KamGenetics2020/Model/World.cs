...
namespace KamGenetics2020.Model
{
    ...
    public class World
    {
        ...
        public static int DefaultConsumptionPerPeriod = 100;
        public static int DefaultStorageCapacity = DefaultConsumptionPerPeriod * 10;
        public static int InitialStorageLevel = DefaultConsumptionPerPeriod * 5;
        public static int ExpectedOrganismLifetimeConsumption = DefaultConsumptionPerPeriod * 80;
        ...
        {
            var minReplenishment = MinPopulationToSupport * World.DefaultConsumptionPerPeriod;
            var maxReplenishment = MaxPopulationToSupport * World.DefaultConsumptionPerPeriod;
        ...
        private int CurrentDyingCount => Organisms.Count(o => o.IsTerminated);
        ...
        {
            ThisPeriodStats.Terminated = CurrentDyingCount;
            LastDyingCount = CurrentDyingCount;
            // Adding to a public list for db persistence
            DeadOrganisms.AddRange(Organisms.Where(r => r.IsTerminated));
            Organisms.RemoveAll(r => r.IsTerminated);
        }
        ...
        public double OrganismStealsFood(Organism thief, double killProbability)
        {
            if (Population < 2)
            {
            ...
            var curIdx = Organisms.IndexOf(thief);
            ...
            for (int i = 0; i < victimCount; i++)
            {
                ...
                do
                    foundIdx = RandomHelper.StandardGeneratorInstance.Next(0, Population);
                while (curIdx == foundIdx);
                if (CanSteal(thief, Organisms[foundIdx], killProbability, out killsVictim))
                {
        ...
        /// Suppose a thief is in proximity of no more than 10% of the population.
        /// But also suppose due to time constraints he cannot approach more than 
        /// certain maximum number.
        /// 
        /// </summary>
        /// <returns></returns>
        private static int GetVictimCount()
        {
            const int maxVictims = 13;
            return Math.Max(maxVictims, MaxPopulationToSupport / 10);
        }

        private bool CanSteal(Organism thief, Organism victim, double killProbability, out bool killsVictim)
        {
            killsVictim = false;
